...
use Request;
use Image;
class uploadplan extends Controller{
  ...
  public static function main(){
    ...
    $show_folder_url = $root_url . '/upload_file/venue/thumbnail/';
    $link_folder_url = $root_url . '/upload_file/venue/default/';
    ...
    if (isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
        ...
        if ($image_res) {
            ...
            if($role_method == "new_image"){
                $new_file_name = $image_name_only . '_' . rand(0, 9999999999) . '.' . $image_extension;
                $destination_output = "#add_new_room";
            }else{
                $room_id = Request::input('room_id');
                $new_file_name = "room_plan_".$room_id.".".$image_extension;
                $destination_output = "#room_id_".$room_id;
            }

            //folder path to save resized images and thumbnails
            $thumb_save_folder = '..' . $root_url . '/upload_file/venue/thumbnail/' . $new_file_name;
            $image_save_folder = $destination_folder . $new_file_name;

            //call normal_resize_image() function to proportionally resize image
            if (uploadplan::normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality)) {
                //call crop_image_square() function to create square thumbnails
                if (!uploadplan::crop_image_square($image_res, $thumb_save_folder, $image_type, $thumb_square_size, $image_width, $image_height, $jpeg_quality)) {
                ...
                //echo '<img src="'. $show_folder_url . $thumb_prefix . $new_file_name . '" alt="Thumbnail">';
                //echo '<br />';
                //echo '<img src="'. $destination_folder . $new_file_name . '" alt="Resized Image">';
                //echo '</div>';
    ...
    public static function normal_resize_image($source, $destination, $image_type, $max_size, $image_width, $image_height, $quality) {
        ...
        if ($image_width <= $max_size && $image_height <= $max_size) {
            if (uploadplan::save_image($source, $destination, $image_type, $quality)) {
                return true;
            }
        }
        ...
        if (imagecopyresampled($new_canvas, $source, 0, 0, 0, 0, $new_width, $new_height, $image_width, $image_height)) {
            uploadplan::save_image($new_canvas, $destination, $image_type, $quality); //save resized image
        }
  ...
  public static function crop_image_square($source, $destination, $image_type, $square_size, $image_width, $image_height, $quality) {
      ...
      if (imagecopyresampled($new_canvas, $source, 0, 0, $x_offset, $y_offset, $square_size, $square_size, $s_size, $s_size)) {
          uploadplan::save_image($new_canvas, $destination, $image_type, $quality);
      }
